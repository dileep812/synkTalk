  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
      <aside className="rounded-2xl border border-slate-200 bg-slate-50 p-4 self-start">
        <label htmlFor="dashboard-search-requests" className="block text-sm font-semibold text-slate-700">
          Find requests
        </label>
        <input
          id="dashboard-search-requests"
          type="text"
          value={searchInput}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
        ...
        {loading ? (
          <p className="mt-4 text-sm font-medium text-slate-600 animate-pulse">Loading invites...</p>
          ...
          <p className="mt-4 text-sm font-medium text-slate-500 bg-slate-50 border border-dashed border-slate-200 rounded-xl p-6 text-center">
            {searchInput.trim() ? `No requests found for "${searchInput}".` : 'No pending invites.'}
          </p>
        ) : (
          <ul className="mt-4 space-y-3">
            {pendingRequests.map((targetUser) => {
